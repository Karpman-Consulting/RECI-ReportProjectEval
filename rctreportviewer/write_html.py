...
def write_html_file(rct_detailed_report):
    with open(rct_detailed_report.output_file_path, "w", encoding="utf-8") as file:
        ...
        write_project_header(file, rct_detailed_report)
        write_model_component_summary(file, rct_detailed_report)
        ...
        write_report_outcomes(file, rct_detailed_report)
        write_misc_elements(file)
        write_html_footer(file)

def write_html_head(file):
    file.write("""
    <!DOCTYPE HTML>
...
def write_project_header(file, report):
    file.write(f"""
    <div class="d-flex flex-nowrap">
        <div class="flex-grow-1">
            <h1 class="text-center mb-4">RECI - Project Evaluation Report</h1>
            <div class="mb-3">
                <p><strong>Ruleset:</strong> {report.evaluation_data["ruleset"]}</p>
                <p><strong>Generated on:</strong> {report.evaluation_data["date_run"]}</p>
                <p><strong>Models Analyzed:</strong> {", ".join(report.model_types)}</p>
            </div>
    """)

def write_model_component_summary(file, report):
    tooltip_lines = []
    for system_type, systems in report.hvac_system_types_b.items():
        tooltip_lines.append(f"<div class='text-start'><b>{system_type}</b>: {len(systems)}</div>")
    tooltip_html = "".join(tooltip_lines)

    baseline = report.baseline_model_summary
    proposed = report.proposed_model_summary

    def fmt(val):
        return f"{round(val):,}" if isinstance(val, (int, float)) else val

    def fmt_list(lst):
        return ", ".join(s.title() for s in lst)

    file.write(f"""
    <div class="mb-3 me-4">
        <button class="btn btn-info collapsed" type="button" data-bs-toggle="collapse"
                data-bs-target="#collapse-model-component-summary" aria-expanded="false">
                ...
                <table class="table table-sm table-borderless" style="width: 400px;">
                    <thead>
                        <tr style="border-bottom: 2px solid black;">
                            <th class="col-4 text-end"></th>
                            <th class="col-4 text-center">Baseline</th>
                            <th class="col-4 text-center">Proposed</th>
                        </tr>
                    </thead>
                    <tbody style="font-size: 12px;">
                        <tr><td class="text-end">Building Qty</td><td class="text-center">{baseline['building_count']}</td><td class="text-center">{proposed['building_count']}</td></tr>
                        <tr><td class="text-end">Total Floor Area</td><td class="text-center">{fmt(baseline['total_floor_area'])}</td><td class="text-center">{fmt(proposed['total_floor_area'])}</td></tr>
                        <tr><td class="text-end">Building Area Qty</td><td class="text-center">{baseline['building_segment_count']}</td><td class="text-center">{proposed['building_segment_count']}</td></tr>
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="d-inline-block" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" data-bs-title="{tooltip_html}" style="text-decoration: underline dotted; text-underline-offset: 3px; cursor: help;">
                                    {baseline['system_count']}
                                </span>
                            </td>
                            <td class="text-center">{proposed['system_count']}</td>
                        </tr>
                        <tr><td class="text-end">Zone Qty</td><td class="text-center">{baseline['zone_count']}</td><td class="text-center">{proposed['zone_count']}</td></tr>
                        <tr><td class="text-end">Space Qty</td><td class="text-center">{baseline['space_count']}</td><td class="text-center">{proposed['space_count']}</td></tr>
                        <tr><td class="text-end">Fluid Loops</td><td class="text-center">{fmt_list(baseline['fluid_loop_types'])}</td><td class="text-center">{fmt_list(proposed['fluid_loop_types'])}</td></tr>
                        <tr><td class="text-end">Pump Qty</td><td class="text-center">{baseline['pump_count']}</td><td class="text-center">{proposed['pump_count']}</td></tr>
                        <tr><td class="text-end">Boiler Qty</td><td class="text-center">{baseline['boiler_count']}</td><td class="text-center">{proposed['boiler_count']}</td></tr>
                        <tr><td class="text-end">Chiller Qty</td><td class="text-center">{baseline['chiller_count']}</td><td class="text-center">{proposed['chiller_count']}</td></tr>
                        <tr><td class="text-end">Heat Rejection Qty</td><td class="text-center">{baseline['heat_rejection_count']}</td><td class="text-center">{proposed['heat_rejection_count']}</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
def write_compliance_calculations(file, report):
    file.write(f"""
    <div class="mb-3 me-4">
        <button class="btn btn-info collapsed" type="button" data-bs-toggle="collapse"
                data-bs-target="#collapse-compliance-calcs" aria-expanded="false">
    ...
    for (
        energy_source,
        proposed_energy_use,
    ) in report.proposed_model_summary.get(
        "energy_by_fuel_type", {}
    ).items():
        baseline_energy_use = report.baseline_model_summary.get(
            "energy_by_fuel_type", {}
        ).get(energy_source, 0)
        baseline_unregulated_energy = (
            report.baseline_model_summary.get(
    ...
    output = report.rpd_data.get("output", {})
    baseline_compliance_calcs = report.baseline_model_summary.get("compliance_calcs_by_parameter", {})
    proposed_compliance_calcs = report.proposed_model_summary.get("compliance_calcs_by_parameter", {})
    ...
    file.write(
        ...
        """)
    if "ASHRAE 90.1-2022" in report.ruleset:
        file.write(
            f"""
                                    <tr style="font-size: 12px;" class="lh-1 text-center">
    ...
    file.write(
        f"""
                                    ...
                                    <tr style="font-size: 12px;" class="lh-1 text-center">
                                        ...
                                        <td id="bpf_energy_cost">{round(output.get("total_area_weighted_building_performance_factor", report.bpfs_by_metric["Cost"]), 2)}</td>
                                        <td id="bpf_site_energy">{round(report.bpfs_by_metric["Site Energy"], 2)}</td>
                                        <td id="bpf_source_energy">{round(report.bpfs_by_metric["Source Energy"], 2)}</td>
                                        <td id="bpf_ghg_emissions">{round(report.bpfs_by_metric["GHG Emissions"], 2)}</td>
                                    </tr>
                                    <tr style="font-size: 12px;" class="lh-1 text-center">
                                    ...
                                    <tr style="font-size: 12px;" class="lh-1 text-center">
                                        ...
                                        <td id="pci_nre_cost">{round(output.get("total_proposed_building_energy_cost_excluding_renewable_energy", 0) / output.get("baseline_building_performance_energy_cost"), 2):,}</td>
...
def write_results_summary(file, report):
    file.write(f"""
    <div class="mb-3 me-4">
    ...
    # JavaScript for charts and toggles
    file.write(
        f"""
        <script>
            ...
            document.addEventListener("DOMContentLoaded", () => {{
                ...
                // Chart labels
                const labels = {[label.replace('_', ' ').title() for label in report.baseline_model_summary["elec_by_end_use"].keys()]};

                const elecDataRaw = {{
                    consumption: {{
                        baseline: {list(report.baseline_model_summary["elec_by_end_use"].values())},
                        proposed: {list(report.proposed_model_summary["elec_by_end_use"].values())}
                    }},
                    eui: {{
                        baseline: {list(report.baseline_model_summary["elec_by_end_use_eui"].values())},
                        proposed: {list(report.proposed_model_summary["elec_by_end_use_eui"].values())}
                    }}
                }};

                const gasDataRaw = {{
                    consumption: {{
                        baseline: {list(report.baseline_model_summary["gas_by_end_use"].values())},
                        proposed: {list(report.proposed_model_summary["gas_by_end_use"].values())}
                    }},
                    eui: {{
                        baseline: {list(report.baseline_model_summary["gas_by_end_use_eui"].values())},
                        proposed: {list(report.proposed_model_summary["gas_by_end_use_eui"].values())}
                    }}
                }};

                const energyDataRaw = {{
                    consumption: {{
                        baseline: {list(report.baseline_model_summary["energy_by_end_use"].values())},
                        proposed: {list(report.proposed_model_summary["energy_by_end_use"].values())}
                    }},
                    eui: {{
                        baseline: {list(report.baseline_model_summary["energy_by_end_use_eui"].values())},
                        proposed: {list(report.proposed_model_summary["energy_by_end_use_eui"].values())}
                    }}
                }};
                ...
                const elecData = {{
                    ...
                    datasets: [
                        {{
                            label: 'Baseline',
                            data: {list(report.baseline_model_summary["elec_by_end_use"].values())},
                            backgroundColor: 'rgba(54, 162, 235, 0.7)'
                        }},
                        {{
                            label: 'Proposed',
                            data: {list(report.proposed_model_summary["elec_by_end_use"].values())},
                ...
                const gasData = {{
                    ...
                    datasets: [
                        {{
                            label: 'Baseline',
                            data: {list(report.baseline_model_summary["gas_by_end_use"].values())},
                            backgroundColor: 'rgba(255, 180, 80, 0.5)'
                        }},
                        {{
                            label: 'Proposed',
                            data: {list(report.proposed_model_summary["gas_by_end_use"].values())},
                ...
                const energyData = {{
                    ...
                    datasets: [
                        {{
                            label: 'Baseline',
                            data: {list(report.baseline_model_summary["energy_by_end_use"].values())},
                            backgroundColor: 'rgba(128, 0, 64, 0.6)'
                        }},
                        {{
                            label: 'Proposed',
                            data: {list(report.proposed_model_summary["energy_by_end_use"].values())},
                ...
                function updateTotals(source, unitType) {{
                    ...
                    if (source === 'elec') {{
                        baseline = unitType === 'eui'
                            ? {list(report.baseline_model_summary["elec_by_end_use_eui"].values())}
                            : {list(report.baseline_model_summary["elec_by_end_use"].values())};

                        proposed = unitType === 'eui'
                            ? {list(report.proposed_model_summary["elec_by_end_use_eui"].values())}
                            : {list(report.proposed_model_summary["elec_by_end_use"].values())};

                    }} else if (source === 'gas') {{
                        baseline = unitType === 'eui'
                            ? {list(report.baseline_model_summary["gas_by_end_use_eui"].values())}
                            : {list(report.baseline_model_summary["gas_by_end_use"].values())};

                        proposed = unitType === 'eui'
                            ? {list(report.proposed_model_summary["gas_by_end_use_eui"].values())}
                            : {list(report.proposed_model_summary["gas_by_end_use"].values())};

                    }} else if (source === 'energy') {{
                        baseline = unitType === 'eui'
                            ? {list(report.baseline_model_summary["energy_by_end_use_eui"].values())}
                            : {list(report.baseline_model_summary["energy_by_end_use"].values())};

                        proposed = unitType === 'eui'
                            ? {list(report.proposed_model_summary["energy_by_end_use_eui"].values())}
                            : {list(report.proposed_model_summary["energy_by_end_use"].values())};
                    }}
...
def write_envelope_summary(file, report):
    file.write(f"""
    <div class="mb-3 me-4">
    ...
    for building_segment_id in report.baseline_model_summary[
        "total_floor_area_by_building_segment"
    ]:
        if building_segment_id in report.baseline_model_summary["total_roof_area_by_building_segment"]:
            file.write(f"""
                        <tr style="font-size: 12px;" class="lh-1 text-center">
                            <td>{building_segment_id}</td>
                            <td style="border-right: 2px solid black;">Roof</td>
                            <td>{round(report.baseline_model_summary['total_roof_area_by_building_segment'].get(building_segment_id, 0) - report.baseline_model_summary['total_skylight_area_by_building_segment'].get(building_segment_id, 0)):,}</td>
                            <td>{round((report.baseline_model_summary['total_roof_area_by_building_segment'].get(building_segment_id, 0) - report.baseline_model_summary['total_skylight_area_by_building_segment'].get(building_segment_id, 0)) / report.baseline_model_summary['total_roof_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.baseline_model_summary["overall_roof_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.baseline_model_summary["total_skylight_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round(report.baseline_model_summary["total_skylight_area_by_building_segment"].get(building_segment_id, 0) / report.baseline_model_summary['total_roof_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td style="border-right: 2px solid black;">{round(report.baseline_model_summary["overall_skylight_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.proposed_model_summary["total_roof_area_by_building_segment"].get(building_segment_id, 0) - report.proposed_model_summary['total_skylight_area_by_building_segment'].get(building_segment_id, 0)):,}</td>
                            <td>{round((report.proposed_model_summary["total_roof_area_by_building_segment"].get(building_segment_id, 0) - report.proposed_model_summary['total_skylight_area_by_building_segment'].get(building_segment_id, 0)) / report.proposed_model_summary['total_roof_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.proposed_model_summary["overall_roof_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.proposed_model_summary["total_skylight_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round(report.proposed_model_summary["total_skylight_area_by_building_segment"].get(building_segment_id, 0) / report.proposed_model_summary['total_roof_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.proposed_model_summary["overall_skylight_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                        </tr>
                """)
        if building_segment_id in report.baseline_model_summary["total_wall_area_by_building_segment"]:
            file.write(f"""
                        <tr style="font-size: 12px;" class="lh-1 text-center">
                            <td>{building_segment_id}</td>
                            <td style="border-right: 2px solid black;">Ext. Wall</td>
                            <td>{round(report.baseline_model_summary['total_wall_area_by_building_segment'].get(building_segment_id, 0) - report.baseline_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round((report.baseline_model_summary['total_wall_area_by_building_segment'].get(building_segment_id, 0) - report.baseline_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)) / report.baseline_model_summary['total_wall_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.baseline_model_summary["overall_wall_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.baseline_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round(report.baseline_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0) / report.baseline_model_summary['total_wall_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td style="border-right: 2px solid black;">{round(report.baseline_model_summary["overall_window_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.proposed_model_summary["total_wall_area_by_building_segment"].get(building_segment_id, 0) - report.proposed_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round((report.proposed_model_summary["total_wall_area_by_building_segment"].get(building_segment_id, 0) - report.proposed_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)) / report.proposed_model_summary['total_wall_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.proposed_model_summary["overall_wall_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                            <td>{round(report.proposed_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0)):,}</td>
                            <td>{round(report.proposed_model_summary["total_window_area_by_building_segment"].get(building_segment_id, 0) / report.proposed_model_summary['total_wall_area_by_building_segment'][building_segment_id] * 100, 1)}</td>
                            <td>{round(report.proposed_model_summary["overall_window_u_factor_by_building_segment"].get(building_segment_id, 0), 3)}</td>
                        </tr>
            """)
    file.write(f"""
                    </tbody>
...
def write_internal_loads_summary(file, report):
    ...
    for space_type in report.baseline_model_summary["total_floor_area_by_space_type"]:
        file.write(f"""
                                <tr style="font-size: 12px;" class="lh-1 text-center">
                                    <td>{space_type.replace('_', ' ').title()}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_floor_area_by_space_type'].get(space_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_floor_area_by_space_type'][space_type] / report.baseline_model_summary['total_occupants_by_space_type'].get(space_type, math.inf))}</td>
                                    <td>{round(report.baseline_model_summary['total_miscellaneous_equipment_power_by_space_type'].get(space_type, 0) / report.baseline_model_summary['total_floor_area_by_space_type'][space_type], 2)}</td>
                                    <td>{round(report.baseline_lighting_power_allowance_by_space_type.get(space_type, 0) / report.baseline_model_summary['total_floor_area_by_space_type'][space_type], 2)}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_lighting_power_by_space_type'].get(space_type, 0) / report.baseline_model_summary['total_floor_area_by_space_type'][space_type], 2)}</td>
                                    <td>{round(report.proposed_model_summary['total_floor_area_by_space_type'][space_type] / report.proposed_model_summary['total_occupants_by_space_type'].get(space_type, math.inf))}</td>
                                    <td>{round(report.proposed_model_summary['total_miscellaneous_equipment_power_by_space_type'].get(space_type, 0) / report.proposed_model_summary['total_floor_area_by_space_type'][space_type], 2)}</td>
                                    <td>{round(report.proposed_model_summary['total_lighting_power_by_space_type'].get(space_type, 0) / report.proposed_model_summary['total_floor_area_by_space_type'][space_type], 2)}</td>
                                </tr>
            """)
    file.write(f"""
                                <tr  style="font-size: 12px; border-top: 1px solid black;" class="lh-1 fw-bold text-center">
                                    <td>Total</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_floor_area']):,}</td>
                                    <td>{round(report.baseline_model_summary['total_floor_area'] / report.baseline_model_summary['total_occupants'], 2)}</td>
                                    <td>{round(report.baseline_model_summary['total_equipment_power'] / report.baseline_model_summary['total_floor_area'], 2)}</td>
                                    <td>{round(report.baseline_total_lighting_power_allowance / report.baseline_model_summary['total_floor_area'], 2)}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_lighting_power'] / report.baseline_model_summary['total_floor_area'], 2)}</td>
                                    <td>{round(report.proposed_model_summary['total_floor_area'] / report.proposed_model_summary['total_occupants'], 2)}</td>
                                    <td>{round(report.proposed_model_summary['total_equipment_power'] / report.proposed_model_summary['total_floor_area'], 2)}</td>
                                    <td>{round(report.proposed_model_summary['total_lighting_power'] / report.proposed_model_summary['total_floor_area'], 2)}</td>
                                </tr>
                            </tbody>
                        </table>
        """)
    ...
    baseline_schedule_summaries = report.baseline_model_summary["schedule_summaries"]
    for schedule_id in baseline_schedule_summaries.keys():
        baseline_schedule_summary = report.baseline_model_summary["schedule_summaries"].get(schedule_id, {})
        proposed_schedule_summary = report.proposed_model_summary["schedule_summaries"].get(schedule_id, {})
        file.write(f"""
                                <tr style="font-size: 12px;" class="lh-1 text-center">
...
def write_hvac_summary(file, report):
    file.write(f"""
        <div class="mb-3 me-4">
    ...
    if (
            report.proposed_model_summary["chiller_count"]
            + report.baseline_model_summary["chiller_count"]
    ) > 0:
        ...
        write_row = False
        for val in [
            report.baseline_model_summary.get("electric_chiller_count", 0),
            report.baseline_model_summary.get("electric_chiller_plant_capacity", 0),
            report.baseline_model_summary.get("cooling_tower_gpm", 0),
            report.baseline_model_summary.get("cooling_tower_hp", 0),
            report.proposed_model_summary.get("electric_chiller_count", 0),
            report.proposed_model_summary.get("electric_chiller_plant_capacity", 0),
            report.proposed_model_summary.get("cooling_tower_gpm", 0),
            report.proposed_model_summary.get("cooling_tower_hp", 0),
        ]:
            if val > 0:
                write_row = True
                break
        if write_row:
            file.write(f"""
                            <tr style="font-size: 12px;" class="text-center">
                                ...
                                <td>{round(report.baseline_model_summary.get("electric_chiller_count", 0)):,}</td>
                                <td>{round(report.baseline_model_summary.get("electric_chiller_plant_capacity", 0), 1):,}</td>
                                <td>{round(report.baseline_model_summary.get("cooling_tower_gpm", 0), 1):,}</td>
                                <td style="border-right: 2px solid black;">{round(report.baseline_model_summary.get("cooling_tower_hp", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("electric_chiller_count", 0)):,}</td>
                                <td>{round(report.proposed_model_summary.get("electric_chiller_plant_capacity", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("cooling_tower_gpm", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("cooling_tower_hp", 0), 1):,}</td>
                            </tr>
                """)
        write_row = False
        for val in [
            report.proposed_model_summary.get("fossil_fuel_chiller_count", 0),
            report.proposed_model_summary.get("fossil_fuel_chiller_plant_capacity", 0.0),
        ]:
            if val > 0:
                write_row = True
                break
        if write_row:
            file.write(f"""
                            <tr style="font-size: 12px;" class="text-center">
                                ...
                                <td style="background: black;"></td>
                                <td style="background: black;"></td>
                                <td>{round(report.proposed_model_summary.get("fossil_fuel_chiller_count", 0)):,}</td>
                                <td>{round(report.proposed_model_summary.get("fossil_fuel_chiller_plant_capacity", 0.0), 1):,}</td>
                                <td style="background: black;"></td>
                                <td style="background: black;"></td>
                            </tr>
                """)
        file.write(f"""
                            <tr style="font-size: 12px; border-top: 1px solid black;" class="fw-bold text-center subtotal">
                                ...
                                <td>{round(report.baseline_model_summary.get("cooling_tower_gpm", 0), 1):,}</td>
                                <td style="border-right: 2px solid black;">{round(report.baseline_model_summary.get("cooling_tower_hp", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("chiller_count", 0)):,}</td>
                                <td>{round((report.proposed_model_summary.get("electric_chiller_plant_capacity", 0) +
                                            report.proposed_model_summary.get("fossil_fuel_chiller_plant_capacity", 0)), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("cooling_tower_gpm", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("cooling_tower_hp", 0), 1):,}</td>
                            </tr>
                        </tbody>
    ...
    if (
        report.proposed_model_summary["boiler_count"]
        + report.baseline_model_summary["boiler_count"]
    ) > 0:
        file.write(f"""
        ...
        write_row = False
        for val in [
            report.proposed_model_summary.get("electric_boiler_count", 0),
            report.proposed_model_summary.get("electric_boiler_plant_capacity", 0.0),
        ]:
            if val > 0:
                write_row = True
                break
        if write_row:
            file.write(f"""
                            <tr style="font-size: 12px;" class="text-center">
                                ...
                                <td>{round(report.proposed_model_summary.get("electric_boiler_count", 0)):,}</td>
                                <td>{round(report.proposed_model_summary.get("electric_boiler_plant_capacity", 0)):,}</td>
                            </tr>
                """)
        ...
        write_row = False
        for val in [
            report.baseline_model_summary.get("fossil_fuel_boiler_count", 0),
            report.baseline_model_summary.get("fossil_fuel_boiler_plant_capacity", 0.0),
            report.proposed_model_summary.get("fossil_fuel_boiler_count", 0),
            report.proposed_model_summary.get("fossil_fuel_boiler_plant_capacity", 0.0),
        ]:
            if val > 0:
                write_row = True
                break
        if write_row:
            file.write(f"""
                            <tr style="font-size: 12px;" class="text-center">
                                <td style="border-right: 2px solid black;">Fossil Fuel</td>
                                <td>{round(report.baseline_model_summary.get("fossil_fuel_boiler_count", 0)):,}</td>
                                <td style="border-right: 2px solid black;">{round(report.baseline_model_summary.get("fossil_fuel_boiler_plant_capacity", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("fossil_fuel_boiler_count", 0)):,}</td>
                                <td>{round(report.proposed_model_summary.get("fossil_fuel_boiler_plant_capacity", 0), 1):,}</td>
                            </tr>
                """)
        file.write(f"""
                            <tr style="font-size: 12px; border-top: 1px solid black;" class="fw-bold text-center subtotal">
                                ...
                                <td>{round(report.baseline_model_summary.get("fossil_fuel_boiler_count", 0)):,}</td>
                                <td style="border-right: 2px solid black;">{round(report.baseline_model_summary.get("fossil_fuel_boiler_plant_capacity", 0), 1):,}</td>
                                <td>{round(report.proposed_model_summary.get("boiler_count", 0)):,}</td>
                                <td>{round((report.proposed_model_summary.get("electric_boiler_plant_capacity", 0) +
                                            report.proposed_model_summary.get("fossil_fuel_boiler_plant_capacity", 0)), 1):,}</td>
                            </tr>
                        </tbody>
                    </table>
            """)
    ...
    write_row = False
    for val in [
        report.baseline_model_summary["heating_capacity_by_fuel_type"].get("Electricity", 0.0),
        report.baseline_model_summary["cooling_capacity_by_fuel_type"].get("Electricity", 0.0),
        report.proposed_model_summary["heating_capacity_by_fuel_type"].get("Electricity", 0.0),
        report.proposed_model_summary["cooling_capacity_by_fuel_type"].get("Electricity", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    <td style="border-right: 2px solid black;">Electricity</td>
                                    <td>{round(report.baseline_model_summary['heating_capacity_by_fuel_type'].get("Electricity", 0.0)):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['cooling_capacity_by_fuel_type'].get("Electricity", 0.0)):,}</td>
                                    <td>{round(report.proposed_model_summary['heating_capacity_by_fuel_type'].get("Electricity", 0.0)):,}</td>
                                    <td>{round(report.proposed_model_summary['cooling_capacity_by_fuel_type'].get("Electricity", 0.0)):,}</td>
                                </tr>
            """)
    # Check if there are any fossil fuel heating capacities in the baseline or proposed models
    write_row = False
    for val in [
        report.baseline_model_summary["heating_capacity_by_fuel_type"].get("Fossil Fuel", 0.0),
        report.proposed_model_summary["heating_capacity_by_fuel_type"].get("Fossil Fuel", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    <td style="border-right: 2px solid black;">Fossil Fuel</td>
                                    <td>{round(report.baseline_model_summary['heating_capacity_by_fuel_type'].get("Fossil Fuel", 0.0)):,}</td>
                                    <td style="background: black; border-right: 2px solid black;"></td>
                                    <td>{round(report.proposed_model_summary['heating_capacity_by_fuel_type'].get("Fossil Fuel", 0.0)):,}</td>
                                    <td style="background: black;"></td>
                                </tr>
            """)
    ...
    write_row = False
    for val in [
        report.baseline_model_summary["heating_capacity_by_fuel_type"].get("On-site Boiler Plant", 0.0),
        report.proposed_model_summary["heating_capacity_by_fuel_type"].get("On-site Boiler Plant", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    ...
                                    <td>{round(report.baseline_model_summary['heating_capacity_by_fuel_type'].get("On-site Boiler Plant", 0.0)):,}</td>
                                    <td style="background: black; border-right: 2px solid black;"></td>
                                    <td>{round(report.proposed_model_summary['heating_capacity_by_fuel_type'].get("On-site Boiler Plant", 0.0)):,}</td>
                                    <td style="background: black;"></td>
                                </tr>
            """)
    ...
    write_row = False
    for val in [
        report.baseline_model_summary["heating_capacity_by_fuel_type"].get("Purchased Heat", 0.0),
        report.proposed_model_summary["heating_capacity_by_fuel_type"].get("Purchased Heat", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    ...
                                    <td>{round(report.baseline_model_summary['heating_capacity_by_fuel_type'].get("Purchased Heat", 0.0)):,}</td>
                                    <td style="background: black; border-right: 2px solid black;"></td>
                                    <td>{round(report.proposed_model_summary['heating_capacity_by_fuel_type'].get("Purchased Heat", 0.0)):,}</td>
                                    <td style="background: black;"></td>
                                </tr>
            """)
    ...
    write_row = False
    for val in [
        report.baseline_model_summary["cooling_capacity_by_fuel_type"].get("On-site Chiller Plant", 0.0),
        report.proposed_model_summary["cooling_capacity_by_fuel_type"].get("On-site Chiller Plant", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    ...
                                    <td style="background: black;"></td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['cooling_capacity_by_fuel_type'].get("On-site Chiller Plant", 0.0)):,}</td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['cooling_capacity_by_fuel_type'].get("On-site Chiller Plant", 0.0)):,}</td>
                                </tr>
            """)
    # Check if there are any Purchased CHW cooling capacities in the baseline or proposed models
    write_row = False
    for val in [
        report.baseline_model_summary["cooling_capacity_by_fuel_type"].get("Purchased CHW", 0.0),
        report.proposed_model_summary["cooling_capacity_by_fuel_type"].get("Purchased CHW", 0.0),
    ]:
        if val > 0:
            write_row = True
            break
    if write_row:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    ...
                                    <td style="background: black;"></td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['cooling_capacity_by_fuel_type'].get("Purchased CHW", 0.0)):,}</td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['cooling_capacity_by_fuel_type'].get("Purchased CHW", 0.0)):,}</td>
                                </tr>
            """)
    file.write(f"""
                                <tr style="font-size: 12px; border-top: 1px solid black;" class="fw-bold text-center subtotal">
                                    <td style="border-right: 2px solid black;">Total</td>
                                    <td>{round(report.baseline_model_summary['heating_capacity_by_fuel_type'].get("Total", 0.0)):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['cooling_capacity_by_fuel_type'].get("Total", 0.0)):,}</td>
                                    <td>{round(report.proposed_model_summary['heating_capacity_by_fuel_type'].get("Total", 0.0)):,}</td>
                                    <td>{round(report.proposed_model_summary['cooling_capacity_by_fuel_type'].get("Total", 0.0)):,}</td>
                                </tr>
                            </tbody>
                        </table>
        """)
    ...
    file.write(f"""
                       ...
                       <p><strong>Outdoor Airflow:</strong> {round(report.baseline_model_summary['total_zone_minimum_oa_flow']):,} CFM</p>
    ...
    for fan_type in ["Supply", "Return/Relief", "Exhaust", "Zonal Exhaust"]:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    ...
                                    <td>{round(report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0) / (report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("CONSTANT", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0) / (report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0) / (report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0) / (report.baseline_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.baseline_model_summary['other_air_flow_by_fan_type'].get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['other_fan_power_by_fan_type'].get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.baseline_model_summary['other_fan_power_by_fan_type'].get(fan_type, 0) / (report.baseline_model_summary['other_air_flow_by_fan_type'].get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.baseline_model_summary['total_air_flow_by_fan_type'].get(fan_type, 0)):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / 1000, 2):,}</td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / (report.baseline_model_summary['total_air_flow_by_fan_type'].get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(100 * report.baseline_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / sum(report.baseline_model_summary["total_fan_power_by_fan_type"].values()))}</td>
                                </tr>
            """)
    # --------- Subtotal Row -------------
    file.write(f"""
    ...
    file.write(f"""
                                <tr style="font-size: 12px; border-top: 1px solid black;" class="text-center">
                                    ...
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['other_fan_power_by_fan_type'].get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.baseline_model_summary['total_fan_power_by_fan_type'].get("Terminal Unit", 0)):,}</td>
                                    <td style="background: black;"></td>
                                    <td style="background: black;"></td>
                                </tr>
                            </tbody>
    ...
    file.write(f"""
                       ...
                       <p><strong>Outdoor Airflow:</strong> {round(report.proposed_model_summary['total_zone_minimum_oa_flow']):,} CFM</p>
                       <table class="table table-sm table-borderless fan-summary" style="width: 1250px;">
                           <thead>
                               <tr class="text-center">
                                   <th style="border: 2px solid black; width: 12%;" rowspan="2">Fan Type</th>
    ...
    for fan_type in ["Supply", "Return/Relief", "Exhaust", "Zonal Exhaust"]:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
                                    <td style="border-right: 2px solid black;">{fan_type}</td>
                                    <td>{round(report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get(fan_type, 0) / (report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("CONSTANT", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get(fan_type, 0) / (report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get(fan_type, 0) / (report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get(fan_type, 0) / (report.proposed_model_summary['total_air_flow_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.proposed_model_summary['other_air_flow_by_fan_type'].get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['other_fan_power_by_fan_type'].get(fan_type, 0) / 1000, 2):,}</td>
                                    <td style="border-right: 2px solid black;">{round(report.proposed_model_summary['other_fan_power_by_fan_type'].get(fan_type, 0) / (report.proposed_model_summary['other_air_flow_by_fan_type'].get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(report.proposed_model_summary['total_air_flow_by_fan_type'].get(fan_type, 0)):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / 1000, 2):,}</td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / (report.baseline_model_summary['total_air_flow_by_fan_type'].get("Supply", 99999999) or 99999999), 4)}</td>
                                    <td>{round(100 * report.proposed_model_summary['total_fan_power_by_fan_type'].get(fan_type, 0) / sum(report.proposed_model_summary["total_fan_power_by_fan_type"].values()))}</td>
                                </tr>
            """)
    # --------- Subtotal Row -------------
    file.write(f"""
    ...
    file.write(f"""
                                <tr style="font-size: 12px; border-top: 1px solid black;" class="text-center">
                                    <td style="border-right: 2px solid black;">Terminal Units</td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("CONSTANT", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("VARIABLE_SPEED_DRIVE", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("MULTISPEED", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_control_by_fan_type'].get("Constant Cycling", {}).get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['other_fan_power_by_fan_type'].get("Terminal Unit", 0)):,}</td>
                                    <td style="border-right: 2px solid black; background: black;"></td>
                                    <td style="background: black;"></td>
                                    <td>{round(report.proposed_model_summary['total_fan_power_by_fan_type'].get("Terminal Unit", 0)):,}</td>
                                    <td style="background: black;"></td>
                                    <td style="background: black;"></td>
                                </tr>
                            </tbody>
...
def write_hvac_details(file, report):
    ...
    for system_summary in report.baseline_model_summary["hvac_system_summaries"]:
        file.write(f"""
                                <tr style="font-size: 12px;" class="text-center">
...
def write_report_outcomes(file, report):
    ...
    rule_categories = {
        "Failing": report.rules_failed,
        "Passing": report.rules_passed,
        "Undetermined": report.full_eval_rules_undetermined
                        + report.appl_eval_rules_undetermined,
        "N/A": report.rules_not_applicable,
    }
    ...
    for category, rules in rule_categories.items():
        ...
        if category == "Undetermined":
            sections_seen = set()
            for rule_id in report.full_eval_rules_undetermined:
                rule_data = next(
                    rule
                    for rule in report.evaluation_data["rules"]
                ...
                outcome_summary = " | ".join(
                    [
                        f"{k}: {v}"
                        for k, v in report.rule_evaluation_outcome_counts[
                        rule_id
                    ].items()
                    ]
                )
            ...
            sections_seen = set()
            for rule_id in report.appl_eval_rules_undetermined:
                rule_data = next(
                    rule
                    for rule in report.evaluation_data["rules"]
                    if rule["rule_id"] == rule_id
